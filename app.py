...
## 2. Polling-Based Solution (Your Original Code, Simplified)

If you must stick to the polling method, here is your code with the redundant `socket.timeout` import removed and the main `if __name__ == '__main__':` block simplified for standard deployment.

```python:Shopify Polling App (Corrected Threading):app.py
import os
import requests
import json
import smtplib
            ...
            if check_shopify_stock(variant_id):
                ...
                for email in emails:
                    ...
                    if send_email(email, notification_subject, notification_body):
                        ...
                        print(f"⚠️ WARNING: Failed to send notification email to {email} for variant {variant_id}.")
